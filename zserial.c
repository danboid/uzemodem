...
// Map ZMODEM names to Uzebox UART functions
#define UartGetChar  UartReadChar
#define UartPutChar  UartSendChar

// Map ZMODEM's expected frame counter function to Uzebox's actual counter
#define GetFrameCount GetVsyncCounter

static uint8_t in_32bit_block = 0;

static ZRESULT zm_await_zdle(void);
    ...
    if (IS_ERROR(c1)) {
      ...
      if (IS_ERROR(c2)) {
        ...
        if (IS_ERROR(b)) {
          ...
          TRACEF("Byte %d; hdr at 0x%04x; ptr at 0x%04x\n", i, (uint16_t)hdr, (uint64_t)ptr);
          *ptr++ = (uint8_t)b;
...
ZRESULT zm_send_pos_hdr(uint8_t type, uint32_t pos) {
  ...
  hdr.type = type;
  // ZMODEM expects position in Big-Endian (Least Significant Byte in f0/p0)
  // We manually shift to ensure correctness regardless of CPU architecture
  hdr.flags.f0 = (uint8_t)(pos & 0xFF);
  hdr.flags.f1 = (uint8_t)((pos >> 8) & 0xFF);
  hdr.flags.f2 = (uint8_t)((pos >> 16) & 0xFF);
  hdr.flags.f3 = (uint8_t)((pos >> 24) & 0xFF);
